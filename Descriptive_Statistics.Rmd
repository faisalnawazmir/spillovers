...
fit_val_diesel_ma<-log_ret_dt$Diesel_log_ret-diesel_ma$residuals
log_ret_dt <- log_ret_dt%>%mutate(fit_val_diesel_ma= fit_val_diesel_ma)
stargazer(diesel_ma, header=FALSE,type="html")
plt<-ggplot(log_ret_dt,aes(x=Date,y=Diesel_log_ret))+
  geom_line()
plt+geom_line(log_ret_dt, mapping= aes(x=Date,y=fit_val_diesel_ma), col =2 )
```
