...
aux2<-aux %>% select(-Crude_Oil_Europe_Brent) %>% dplyr::filter(Day != "Sunday") # treating the first 2 sources of NA
NA_dates <- aux2 %>%
...
```{r na_treating_main}
df <- df %>% select(-Crude_Oil_Europe_Brent) %>% dplyr::filter(Day != "Sunday")
NA_dates <- df %>%
  filter_at(vars(-c(Date,Day)), all_vars(is.na(.))) %>%
  pull(Date)
df <- df %>% dplyr::filter(!Date %in% NA_dates)
df <- df %>% mutate_all(~ replace(., is.na(.), mean(., na.rm = TRUE)))
```
...
```{r summary_table_draft}

summary_df<-long_log_ret %>% group_by(as.factor(variable)) %>%
  get_summary_stats(type = "common") %>%select(-variable,n) %>% 
  rename("variable" = 1)%>%
...
summary_df[c("skewness", "kurtosis"),] <- rbind(c(sapply(log_ret_df[c(2:7)],skewness),
                                                  sapply(log_ret_df[c(2:7)],kurtosis),
                                                  sapply(log_ret_df[1000:6000,c(2:7)],kurtosis)))
...
for(i in 2:length(log_ret_df)){
  ...
  #arch_5_list[[i-1]] <- garchFit(~garch(0,5), data =res, trace = F)
}
...
```{r all_tests, echo = F}
res_matrix<-data.frame(matrix(ncol=6,nrow=10))
x <- c("name", "age", "gender")
colnames(res_matrix) <- stylized_names
rownames(res_matrix)<-c("Shapiro Normality","Shapiro Normality PVal","ADF","ADF Pval","Q(10)","Q(10) Pval","Q2(10)","Q2(10) Pval","ARCH(5)","ARCH(5) PVal")

for (i in 1:length(stylized_names)){
  res_matrix[1,i]<-round(shapiro_norm_list[[i]]$statistic,2)
  res_matrix[2,i]<-ifelse(shapiro_norm_list[[i]]$p.value<0.001,"<0.001",shapiro_norm_list[[i]]$p.value)
  res_matrix[3,i]<-round(adf_list[[i]]$statistic,2)
  res_matrix[4,i]<-ifelse(adf_list[[i]]$p.value==0.01,"<0.01",adf_list[[i]]$p.value)
  res_matrix[5,i]<-round(q_10_list[[i]]$statistic,2)
  res_matrix[6,i]<-ifelse(q_10_list[[i]]$p.value<0.001,"<0.001",q_10_list[[i]]$p.value)
  res_matrix[7,i]<-round(q2_10_list[[i]]$statistic,2)
  res_matrix[8,i]<-ifelse(q2_10_list[[i]]$p.value<0.001,"<0.001",q2_10_list[[i]]$p.value)
  arch_coefs<-summary(arch_5_list[[i]])$coef[5,]
  p_val_arch<-2*pt(q=arch_coefs[3], 30, lower=FALSE) 
  res_matrix[9,i]<-round(arch_coefs[1],2)
  res_matrix[10,i]<-ifelse(p_val_arch<0.001,"<0.001",p_val_arch)
}
...
<div align="center">
### AR(1)
```{r ar_res,echo=TRUE,results='asis'}
stargazer(ar_1_list, header=FALSE,type="html",column.labels = stylized_names)
```
</div>
<div align="center">
### MA(1)
```{r ma_res,echo=TRUE,results='asis'}
stargazer(ma_1_list, header=FALSE,type="html",column.labels = stylized_names,font.size = "tiny") # to make font size smaller)
```
</div>
