...
aux2<-aux %>% select(-Crude_Oil_Europe_Brent) %>% dplyr::filter(Day != "Sunday") # treating the first 2 sources of NAs
NA_dates <- aux2 %>%
...
```{r all_tests, echo = F}
res_matrix<-data.frame(matrix(ncol=6,nrow=12))
x <- c("name", "age", "gender")
colnames(res_matrix) <- stylized_names
rownames(res_matrix)<-c("Shapiro Normality","Shapiro Normality PVal","ADF","ADF Pval","Q(10)","Q(10) Pval","Q2(10)","Q2(10) Pval","ARCH(5)","ARCH(5) Pval","AR(1)","AR(1) Pval")
...
for (i in 1:length(stylized_names)){
  ...
  arch_res<-round(coeftest(arch_5_list[[i]]),5)
  res_matrix[9,i]<-round(arch_res[6,1],2)
  res_matrix[10,i]<-ifelse(arch_res[6,4]<0.001,"<0.001",arch_res[6,4])
  ar_res<-round(coeftest(ar_1_list[[i]]),3)
  res_matrix[11,i]<-ar_res[1,1]
  res_matrix[12,i]<-ifelse(ar_res[1,4]<0.001,"<0.001",ar_res[1,4])
}
summary_df %>% rbind(res_matrix)%>%
  kbl() %>%
